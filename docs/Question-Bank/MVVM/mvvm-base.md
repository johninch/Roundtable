...
2. 数组方面的变动，如**根据索引改变元素**，以及**直接改变数组长度**，不能被检测到。例如`vm.items[indexOfItem] = newValue`，`vm.items.length = newLength`。
    - 其实`object.defineProperty`本身是可以监听和响应数组`arr[index] = val`的变动的，但 尤大 说出于从 性能代价和用户体验收益权衡，并没与实现这个功能。

Vue提供了解决方法：传送门！！！

#### 使用 Proxy 代替 Object.defineProperty

传送门：[摒弃 Object.defineProperty，基于 Proxy 的观察者机制探索](https://juejin.im/post/5bf3e632e51d452baa5f7375)
